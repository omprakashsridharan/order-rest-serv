...
# Build application
COPY . .
RUN cargo build --release --bin inventoryserv

# We do not need the Rust toolchain to run the binary!
FROM debian:buster-slim AS runtime
WORKDIR /app
COPY --from=builder /app/target/release/inventoryserv /usr/local/bin/inventoryserv
...
ENTRYPOINT ["/usr/local/bin/inventoryserv"]